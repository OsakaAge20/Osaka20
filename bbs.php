...
----------------------------------------------------------------*/
	...
	//$time3 = time();
	$res = "0";
	if($title == "") error("スレタイは入力してください");
	if($name == ""){
	$name = "ななしさん＠Noname";
	}
	if($comment == "") error("本文は入力必須です。");
	if (strlen($title) > 60) error("スレタイは全角30文字以内で入力してください。");
	if (strlen($name) > 60) error("名前は全角30文字以内で入力してください。");

	if($mail){
	if (strlen($mail) > 60) error("mailは全角30文字以内で入力してください。");
	}

	if (strlen($comment) > 8000) error("本文全角4000文字以内で入力してください。");
	if($pass){
if (!preg_match("/^[a-zA-Z0-9\-]+$/", $pass)) error("編集パスワードは半角英数字20文字以内で入力してください。");
	if (strlen($pass) > 20) error("パスワードは半角英数字20文字以内で入力してください。");
	}
...
if (!preg_match("/^[a-zA-Z0-9\-]+$/", $pass)) error("編集パスワードは半角英数字20文字以内で入力してください。");
	...
	$no2 = $_POST['no2'];
	if(!preg_match("/^[0-9]+$/",$no)) error("スレッドナンバーが不正です");
	if(!preg_match("/^[0-9]+$/",$no2)) error("レスナンバーが不正です");
	...
	$time2 = time();
	//$time3 = time();
	if($pass != $row['pass']) error("編集パスワードが違います");
